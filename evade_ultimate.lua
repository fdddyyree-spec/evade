...
-- Функция зажатия E на время (улучшенная)
local function HoldE(duration)
    print("⌨️ Начинаем зажимать E...")
    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
    
    -- Держим E и периодически повторяем нажатие
    local elapsed = 0
    while elapsed < duration do
        wait(0.5)
        elapsed = elapsed + 0.5
        -- Повторное нажатие для надежности
        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
    end
...
local function AutoRevive()
    ...
    for _, data in ipairs(deadPlayers) do
        ...
        -- Проверяем не воскрешали ли недавно (увеличен интервал)
        if not revivedPlayers[player.UserId] or tick() - revivedPlayers[player.UserId] > 30 then
            ...
            -- ЗАЖИМАЕМ E на 10 секунд (увеличено)
            print("🔧 Зажимаем E на 10 секунд...")
            HoldE(10)
            
            -- Ждем завершения воскрешения
            wait(2)
